    ...
    for epoch in range(1, epochs + 1):
        ...
        for train_data in train_loader:
            ...
            if i % show_interval == 0:
                ...
                with open(record + "train_batch_loss.txt",'a',encoding="gbk") as f:
                    f.write(str(loss / train_size)+"\n")
            ...
            if i % valid_interval == 0:
                ...
                with open(record + "valid_evaluation.txt",'a',encoding="gbk") as f:
                    f.write('{loss} {acc}\n'.format(**evaluation))
                ...
                if i % save_interval == 0:
                    ...
                    save_model_path = os.path.join(config['checkpoints_dir'],
                                                   f'{prefix}_{i:06}_loss{loss}_wight.pt')
                    save_model_path1 = os.path.join(config['checkpoints_dir'],
                                                   f'{prefix}_{i:06}_loss{loss}.pt')
                    torch.save(crnn.state_dict(), save_model_path)
                    torch.save(crnn, save_model_path1)
