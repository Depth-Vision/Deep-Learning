...
data_base_dir="C:\\Users\\29754\\Desktop\\CNN\\RS"#输入文件夹的路径
outfile_dir="C:\\Users\\29754\\Desktop\\CNN\\NS"#输出文件夹的路径C:\Users\29754\Desktop\CNN\Data\train\B1
...
for file in os.listdir(data_base_dir):#遍历目标文件夹图片
    ...
    while(1):
        ...
        if k==13:#按回车键确认处理、保存图片到输出文件夹和读取下一张图片
            processed_number+=1
            out_img_name=outfile_dir+'//'+"5"+file.strip()
            cv2.imwrite(out_img_name,image_gamma_correct)
            print ("The number of photos which were processed is ",processed_number,file.strip())
            break